...
async function testOpenAI(apiKey: string) {
  try {
    ...
    return {
      success: false,
      message: `OpenAI test failed: ${getErrorMessage(error)}`,
    };
  }
}
...
async function testAnthropic(apiKey: string) {
  try {
    ...
    return {
      success: false,
      message: `Anthropic test failed: ${getErrorMessage(error)}`,
    };
  }
}
...
async function testGoogleAI(apiKey: string) {
  try {
    ...
    return {
      success: false,
      message: `Google AI test failed: ${getErrorMessage(error)}`,
    };
  }
}
...
export function createApp() {
  ...
  app.get('/user/setup-status', async (c) => {
    try {
      const user = await getUser(c.req.raw);
      const setupData = (await db.getUserSetup(user.id)) ?? {
  ...
  app.post('/integrations/:category/:provider', async (c) => {
    try {
      const user = await getUser(c.req.raw);
      ...
      switch (provider) {
        case 'openai':
          testResult = await testOpenAI(
            String(integration.api_key ?? integration.apiKey ?? ''),
          );
          break;
        case 'anthropic':
  ...
  app.get('/admin/clients/:clientId', async (c) => {
    try {
      await getUser(c.req.raw);
  ...
  app.put('/admin/clients/:clientId', async (c) => {
    try {
      const user = await getUser(c.req.raw);
      const { clientId } = c.req.param();
  ...
  app.delete('/admin/clients/:clientId', async (c) => {
    try {
      const user = await getUser(c.req.raw);
      const { clientId } = c.req.param();
  ...
  app.post('/admin/clients/:clientId/oauth/:platform', async (c) => {
    try {
      const user = await getUser(c.req.raw);
  ...
  app.delete('/admin/clients/:clientId/oauth/:platform', async (c) => {
    try {
      const user = await getUser(c.req.raw);
      const { clientId, platform } = c.req.param();
  ...
  app.get('/admin/clients/:clientId/campaigns', async (c) => {
    try {
      await getUser(c.req.raw);
      const { clientId } = c.req.param();
    ...
    try {
      ...
      const analytics = {
        ...
        totalRevenue: clients.reduce(
          (sum, client) => sum + Number(client.monthlySpend ?? 0),
          0,
        ),
        totalTokensUsed: clients.reduce(
          (sum, client) => sum + Number(client.tokensUsed ?? 0),
          0,
        ),
        totalCampaigns: clients.reduce(
          (sum, client) => sum + Number(client.campaigns ?? 0),
          0,
        ),
        ...
        topClients: clients
          .slice()
          .sort(
            (a, b) => Number(b.monthlySpend ?? 0) - Number(a.monthlySpend ?? 0),
          )
          .slice(0, 5)
          .map((client) => ({
            id: client.id,
